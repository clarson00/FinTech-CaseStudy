...
for symbol in new_records.keys():
    new_record=[new_records[symbol]["date"],new_records[symbol]["open"],new_records[symbol]["high"],new_records[symbol]["low"],new_records[symbol]["close"]]
    historical_stock_data[symbol].append(new_record)

print("-" * 20)
print("The awesome LAM way")
...
historical_stock_data['AAPL'].append(aapl_new_record)
historical_stock_data['MU'].append(aapl_new_record)
historical_stock_data['AMD'].append(aapl_new_record)
historical_stock_data['TWTR'].append(aapl_new_record)
...
new_rec_list = []
new_rec_list.append(aapl_new_record)
new_rec_list.append(mu_new_record)
new_rec_list.append(amd_new_record)
new_rec_list.append(twtr_new_record)

#print(new_rec_list)
x=0
for record in new_records.keys():
    historical_stock_data[record].append(new_rec_list[x])
    x=+1
